const NoticesAlerts = ({ noticeData }: any) => {
  ...
  const multipleNotices = noticeData.map((data: any) => {
    return (
      <div className=" flex flex-col justify-center mb-3 sm:w-[30%]">
        <p className="text-[#7a7a7a] font-['Share_Tech_Mono']">{data.date}</p>
        <p className="text-[#c9c9c9] sm:text-md font-bold">{data.news}</p>
      </div>
    );
  });
