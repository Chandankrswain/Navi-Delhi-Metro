...
const MetroCards = ({ cardData }: any) => {
  const multiplecards = cardData.map((data: any) => {
    return (
      <div
        style={{
      ...
      >
        ...
        <div className="flex flex-col">
          ...
          <p className="text-white text-[20px] font-semibold">{data.colour}</p>
        </div>
      </div>
    );
  });
  return <div className="flex flex-wrap justify-evenly">{multiplecards}</div>;
};
